  ...
  const generateExcelDownload = async (): Promise<void> => {
    try {
      ...
      worksheet.mergeCells(
        `A${currentRow}:${String.fromCharCode(65 + timeSlots.length)}${currentRow}`,
      );
      ...
      subtitleCell.value =
        "หลักสูตรวิศวกรรมคอมพิวเตอร์และหลักสูตรเทคนิคคอมพิวเตอร์";
      ...
      descriptionCell.value =
        "สาขาวิชาวิศวกรรมไฟฟ้า คณะวิศวกรรมศาสตร์ มหาวิทยาลัยเทคโนโลยีราชมงคลล้านนา ตาก";
        ...
        for (let timeIndex = 0; timeIndex < timeSlots.length; timeIndex++) {
          ...
          if (subject && !cellSkip.has(cellKey)) {
            ...
            const subjectCodeAndName = [
              subject.subjectCode || "",
              stripPartSuffix(subject.subjectName || ""),
            ]
              .filter((text) => text)
              .join(" ");

            const subjectText = [subjectCodeAndName, teacherName]
              .filter((text) => text)
              .join("\n");
            ...
            if (newColspan > 1) {
              ...
              worksheet.mergeCells(
                `${String.fromCharCode(64 + col)}${currentRow}:${String.fromCharCode(64 + endCol)}${currentRow}`,
              );
            }
            ...
            if (oddSubject && !cellSkip.has(oddCellKey)) {
              ...
              const subjectCodeAndName = [
                ...
                stripPartSuffix(oddSubject.subjectName || ""),
              ]
                .filter((text) => text)
                .join(" ");
