...
export default function DownloadButtonTimetable({
  ...
  const getCurriculumInfo = () => {
    if (timetables.length === 0)
      return { curriculum: "หลักสูตรไม่ระบุ", filePrefix: "ตารางเรียน" };
  ...
  const safeMergeCells = (
    ...
    try {
      ...
      console.warn(
        `Failed to merge cells ${range}${description ? ` (${description})` : ""}:`,
  ...
  const createStyledCell = (
    ...
    if (options?.merge) {
      try {
        ...
        console.warn(
          `Failed to merge cells ${options.merge}:`,
          error instanceof Error ? error.message : "Unknown error",
        );
      }
    }
  ...
  const initializeWorksheet = (workbook: ExcelJS.Workbook) => {
    ...
    worksheet.getColumn(1).width = 2.5;
  ...
  const stripPartSuffix = (name: string) =>
    name.replace(/\s*\(ส่วนที่ \d+\)\s*$/, "");
  const baseSection = (section: string) => (section || "").replace(/-\d+$/, "");

  const createDataRows = (
    worksheet: ExcelJS.Worksheet,
    outCreditMap: Map<string, number>,
  ) => {
    ...
    timetables.forEach((item) => {
      if (item.plan) {
        const key = `${item.plan.subjectCode}-${baseSection(item.plan.section)}`;
        if (!uniqueSubjects.has(key)) {
          uniqueSubjects.set(key, {
            ...item.plan,
            subjectName: stripPartSuffix(item.plan.subjectName),
            section: baseSection(item.plan.section),
          ...
          existing.lectureHour += item.plan.lectureHour || 0;
          existing.labHour += item.plan.labHour || 0;
        }
      }
    });
    ...
    for (let row = 3; row <= 14; row++) {
      ...
      if (subjectIndex < subjectsArray.length) {
        ...
        const outCredit = outCreditMap.get(subject.subjectCode) ?? "";
        createStyledCell(worksheet, row, 24, outCredit);
    ...
    const termInfo = currentTermYear
      ? `ภาคเรียนที่ ${currentTermYear.split("/")[0]} ปีการศึกษา ${currentTermYear.split("/")[1]}`
      : "ภาคเรียนที่ x ปีการศึกษา xxxx";
  ...
  const createSummaryRow = (
    ...
    const totalLectureHours = subjectsArray.reduce(
      (sum, subject) => sum + (subject.lectureHour || 0),
      0,
    );
    const totalLabHours = subjectsArray.reduce(
      (sum, subject) => sum + (subject.labHour || 0),
      0,
    );
    const totalOutCredits = subjectsArray.reduce(
      (sum, subject) => sum + (outCreditMap.get(subject.subjectCode) ?? 0),
      0,
    );
    ...
    createStyledCell(worksheet, 15, 24, totalOutCredits || "");
  ...
  const createTimeTableHeaders = (worksheet: ExcelJS.Worksheet) => {
    ...
    createStyledCell(worksheet, 18, 2, "คาบ(ทะเบียนกลาง)", {
      font: { size: 9 },
    });
    ...
    // Build combined hours map for split subjects
    const combinedHoursMap = new Map<
      string,
      { lectureHour: number; labHour: number }
    >();
    timetables.forEach((item) => {
      if (item.plan) {
        const key = `${item.plan.subjectCode}-${baseSection(item.plan.section)}`;
        if (!combinedHoursMap.has(key)) {
          combinedHoursMap.set(key, {
            lectureHour: item.plan.lectureHour || 0,
            labHour: item.plan.labHour || 0,
          });
        } else {
          const existing = combinedHoursMap.get(key)!;
          existing.lectureHour += item.plan.lectureHour || 0;
          existing.labHour += item.plan.labHour || 0;
        }
      }
    ...
    scheduleData.day.forEach((day, dayIndex) => {
      ...
      if (dayTimetables.length > 0) {
        console.log(
          `Day ${dayIndex} (${day}) has ${dayTimetables.length} subjects:`,
          dayTimetables.map(
            (t) => `${t.plan?.subjectCode} (${t.startPeriod}-${t.endPeriod})`,
          ),
        );
      }
      ...
      for (let period = 0; period < 28; period++) {
        ...
        if (dayIndex === 2 && period === 14) {
          ...
          const activityCell = createStyledCell(
            worksheet,
            pvsRow,
            startColIndex,
            "กิจกรรม",
        ...
        const subjectInPeriod = dayTimetables.find(
          (item) => period >= item.startPeriod && period <= item.endPeriod,
        );

        if (period >= 15 && period <= 17 && dayIndex === 0) {
          console.log(
            `Period ${period} (display: ${period + 1}):`,
            subjectInPeriod
              ? `Found ${subjectInPeriod.plan?.subjectCode}`
              : "No subject",
          );
        }
        ...
        if (subjectInPeriod && subjectInPeriod.plan) {
          if (period === subjectInPeriod.startPeriod) {
            const colspan =
              subjectInPeriod.endPeriod - subjectInPeriod.startPeriod + 1;

            const combinedKey = `${subjectInPeriod.plan.subjectCode}-${baseSection(subjectInPeriod.plan.section)}`;
            const combined = combinedHoursMap.get(combinedKey);
            const lectureHour =
              combined?.lectureHour ?? (subjectInPeriod.plan.lectureHour || 0);
            const labHour =
              combined?.labHour ?? (subjectInPeriod.plan.labHour || 0);
            const hourInfo = `ท.${lectureHour} ป.${labHour}`;
            const subjectText = `${subjectInPeriod.plan.subjectCode} ${stripPartSuffix(subjectInPeriod.plan.subjectName)} ${hourInfo}`;
            ...
            if (colspan > 1) {
              ...
              safeMergeCells(
                worksheet,
                mergeRange,
                `subject multiple periods: ${subjectInPeriod.plan.subjectCode} (periods ${subjectInPeriod.startPeriod + 1}-${subjectInPeriod.endPeriod + 1})`,
              );
            } else {
              ...
              safeMergeCells(
                worksheet,
                mergeRange,
                `subject single period: ${subjectInPeriod.plan.subjectCode} (period ${subjectInPeriod.startPeriod + 1})`,
              );
            }
            ...
            console.log(
              `Merged subject: ${subjectInPeriod.plan.subjectCode}, DB periods: ${subjectInPeriod.startPeriod}-${subjectInPeriod.endPeriod}, Excel columns: ${startColLetter}-${endColLetter} (${startColIndex}-${endColIndex})`,
            );
            console.log(`Subject text: "${subjectText}"`);
            console.log(
              `Placed in row ${pvsRow}, column ${startColIndex} (${startColLetter})`,
            );
          } else {
            console.log(
              `Skipping period ${period} (display: ${period + 1}) for subject ${subjectInPeriod.plan.subjectCode} - already merged`,
            );
          }
        } else {
  ...
  const generateExcelDownload = async () => {
    try {
      ...
      // Fetch out_credit from curriculum
      let outCreditMap = new Map<string, number>();
      try {
        const res = await fetch("/api/curriculum");
        if (res.ok) {
          const curricula: { id_sub: string; out_credit: number }[] =
            await res.json();
          curricula.forEach((c) => outCreditMap.set(c.id_sub, c.out_credit));
        }
      } catch (e) {
        console.warn("Could not fetch curriculum data:", e);
      }

      createMainHeaders(worksheet);
      createDataRows(worksheet, outCreditMap);
      createSummaryRow(worksheet, outCreditMap);
