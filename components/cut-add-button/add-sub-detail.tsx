...
export default function AddSubDetail({
  subject,
}: {
  ...
  useEffect(() => {
    if (open && subject) {
      ...
      setSelectedTeacherId(
        subject.teacherId ? String(subject.teacherId) : "null",
      );
      ...
      console.log("Initial state set:", {
        ...
        selectedTeacherId: subject.teacherId
      ...
      const loadFreshData = async () => {
        try {
          ...
          if (res.ok) {
            ...
            setSelectedRoomId(
              freshData.roomId ? String(freshData.roomId) : "NONE",
            );
            setSelectedTeacherId(
              freshData.teacherId ? String(freshData.teacherId) : "null",
            );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const isDVEPlan =
        subjectDetail.planType === "DVE-MSIX" ||
        subjectDetail.planType === "DVE-LVC";

      if (isDVEPlan) {
        const searchResponse = await fetch(
          `/api/subject?subjectCode=${encodeURIComponent(subjectDetail.subjectCode)}&termYear=${encodeURIComponent(subjectDetail.termYear)}&yearLevel=${encodeURIComponent(subjectDetail.yearLevel)}`,
        );

        if (searchResponse.ok) {
          const allSameSubjects = await searchResponse.json();

          const dveSubjects = allSameSubjects.filter(
            (s: any) =>
          ...
          console.log(
            "Updating DVE subjects with same subjectCode:",
            dveSubjects.length,
            "subjects",
          );
          console.log(
            "Subject codes being updated:",
            dveSubjects.map((s: any) => `${s.subjectCode}-${s.planType}`),
          );

          for (const dveSubject of dveSubjects) {
            const updateResponse = await fetch(
              `/api/subject/${dveSubject.id}`,
              {
                method: "PATCH",
                headers: {
            ...
            if (!updateResponse.ok) {
              ...
              throw new Error(
                errorData.error ||
                  `การอัปเดตข้อมูล ${dveSubject.planType} ล้มเหลว`,
              );
            }
          }
        }
              ...
              {subject.dep === "นอกสาขา" ? (
                ...
                >
                  <SelectTrigger>
                    <SelectValue
                      placeholder={
                        loadingTeachers ? "กำลังโหลดอาจารย์..." : "เลือกอาจารย์"
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
