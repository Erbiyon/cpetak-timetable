...
export default function DownloadTeacherButton({
  ...
  const getCurriculumInfo = () => {
    if (timetables.length === 0)
      return { curriculum: "หลักสูตรไม่ระบุ", filePrefix: "ตารางสอน" };
  ...
  const enhancedScheduleData: ScheduleData = {
    ...
    timetables: timetables.map((item) => ({
      ...
      subject: stripPartSuffix(item.plan?.subjectName || "ไม่ระบุ"),
  ...
  const createStyledCell = (
    ...
    if (options?.merge) {
      try {
        ...
        console.warn(
          `Cells ${options.merge} are already merged or overlapping`,
        );
      }
    }
  ...
  const createMainHeaders = (worksheet: ExcelJS.Worksheet) => {
    createStyledCell(worksheet, 1, 1, enhancedScheduleData.title1, {
      merge: "A1:E1",
    });
    ...
    createStyledCell(worksheet, 1, 24, "จำนวน คาบเรียน/สัปดาห์", {
      merge: "X1:AA1",
    });
    createStyledCell(worksheet, 1, 28, "นศ.รอบ(คน)", { merge: "AB1:AC1" });
    createStyledCell(worksheet, 1, 30, "ห้องเรียน", { merge: "AD1:AD2" });

    createStyledCell(worksheet, 2, 1, enhancedScheduleData.title2, {
      merge: "A2:E2",
    });
    const subHeaders = [
      "วิชา",
  ...
  const createDataRows = (worksheet: ExcelJS.Worksheet) => {
    ...
    timetables.forEach((item) => {
      if (item.plan) {
        ...
        if (!uniqueSubjects.has(key)) {
          const periodsPerWeek = timetables
            .filter(
              (t) =>
                ...
                baseSection(t.plan.section) === baseSection(item.plan.section),
            )
          ...
          existing.lectureHour =
            (existing.lectureHour || 0) + (item.plan.lectureHour || 0);
          existing.labHour = (existing.labHour || 0) + (item.plan.labHour || 0);
        }
      }
    });
    ...
    for (let row = 3; row <= 14; row++) {
      ...
      if (subjectIndex < subjectsArray.length) {
        ...
        const getCurriculumInfo = (plan: any) => {
          ...
          const yearValue = plan.yearLevel
            ? plan.yearLevel.toString().replace(/ปี\s*/g, "")
            : "";

          switch (plan.planType) {
            case "TRANSFER":
              return yearValue
                ? `วศ.บ. วค.${yearValue}(ทอ.)`
                : `วศ.บ. วค.1 หรือ 2 หรือ 3(ทอ.)`;
            case "FOUR_YEAR":
              return yearValue
                ? `วศ.บ. วค.${yearValue}(4 ปี)`
                : `วศ.บ. วค.1 หรือ 2 หรือ 3 หรือ 4(4 ปี)`;
            case "DVE-LVC":
            case "DVE-MSIX":
    ...
    const teacherName = selectedTeacher
      ? `${selectedTeacher.tName} ${selectedTeacher.tLastName}`
      : "";
      ...
      for (let period = 0; period < maxPeriodsInDay; period++) {
        ...
        if (subjectInPeriod && subjectInPeriod.plan) {
          if (period === subjectInPeriod.startPeriod) {
            ...
            const subjectText = `${subjectInPeriod.plan.subjectCode} ${stripPartSuffix(subjectInPeriod.plan.subjectName)} ${curriculumInfo}`;
            ...
            if (colspan > 1) {
              ...
              safeMergeCells(
                worksheet,
                `${startCol}${pvsRow}:${endCol}${btechRow}`,
              );
            } else {
              const colLetter = getColumnLetter(3 + period);
              safeMergeCells(
                worksheet,
                `${colLetter}${pvsRow}:${colLetter}${btechRow}`,
              );
            }
  ...
  const addDetailedTimetable = (workbook: ExcelJS.Workbook) => {
    ...
    createStyledCell(
      ...
      4,
      1,
      `อาจารย์: ${enhancedScheduleData.fname} ${enhancedScheduleData.lname}`,
      {
        font: { size: 12 },
    ...
    createStyledCell(
      ...
      4,
      5,
      `ภาคเรียน: ${enhancedScheduleData.term}`,
      {
        font: { size: 12 },
        alignment: { horizontal: "left" },
      },
    ...
    enhancedScheduleData.timetables.forEach((item) => {
      ...
      const timeSlot =
        enhancedScheduleData.timeSlot[item.period - 1] || "ไม่ระบุ";
  ...
  const downloadExcelWithEnhancedName = async (buffer: ArrayBuffer) => {
    ...
    const fileName = `${teacherName}_${termYear}.xlsx`;
