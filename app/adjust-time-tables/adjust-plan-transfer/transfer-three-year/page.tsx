...
export default function TransferThreeYear() {
    ...
    useEffect(() => {
        if (plans.length > 0) {
            ...
            const year3Plans = plans.filter(plan => plan.yearLevel && plan.yearLevel.includes("ปี 3"));
            console.log("วิชาปี 3:", year3Plans.length);
            console.log("ตัวอย่างวิชาปี 3:", year3Plans.length > 0 ? year3Plans[0] : "ไม่พบ");
        }
    }, [plans]);
    ...
    const handleSubjectUpdate = async () => {
        ...
        try {
            ...
            const response1 = await fetch(`/api/subject?termYear=${encodeURIComponent(currentTermYear)}&yearLevel=${encodeURIComponent('ปี 3')}&planType=TRANSFER`);
            if (response1.ok) {
