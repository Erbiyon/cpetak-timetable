...
export default function TransferTwoYear() {
        ...
        async function fetchData() {
            ...
            try {
                ...
                if (termRes.ok) {
                    ...
                    const timetableRes = await fetch(`/api/timetable?termYear=${encodeURIComponent(termData.termYear)}&yearLevel=${encodeURIComponent('ปี 2')}&planType=TRANSFER`);
                    if (timetableRes.ok) {
                    ...
                    const query1 = `/api/subject?termYear=${encodeURIComponent(termData.termYear)}&yearLevel=${encodeURIComponent('ปี 2')}&planType=TRANSFER`;
                ...
                try {
                    ...
                    const response = await fetch('/api/timetable', {
                        ...
                        body: JSON.stringify({
                            planId: subjectId,
                            termYear: termYear || '1',
                            yearLevel: 'ปี 2', // เปลี่ยนตามแต่ละไฟล์
    ...
    const handleSubjectUpdate = async () => {
        ...
        try {
            ...
            const timetableRes = await fetch(`/api/timetable?termYear=${encodeURIComponent(termYear || "")}&yearLevel=${encodeURIComponent('ปี 2')}&planType=TRANSFER`);
            if (timetableRes.ok) {
                const timetableData = await timetableRes.json();
            ...
            const planRes = await fetch(`/api/subject?termYear=${encodeURIComponent(termYear || "")}&yearLevel=${encodeURIComponent('ปี 2')}&planType=TRANSFER`);
            if (planRes.ok) {
