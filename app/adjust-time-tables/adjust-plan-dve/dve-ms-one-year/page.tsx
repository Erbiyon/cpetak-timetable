...
export default function DveMsixOneYear() {
  ...
  const handleSubjectUpdate = useCallback(async () => {
    ...
    try {
      ...
      const timetableRes = await fetch(
        `/api/timetable?termYear=${encodeURIComponent(termYear || "")}&yearLevel=${encodeURIComponent("ปี 1")}&planType=DVE-MSIX`,
      );
      if (timetableRes.ok) {
    ...
    async function fetchData() {
      ...
      try {
        ...
        if (termRes.ok) {
          ...
          const timetableRes = await fetch(
            `/api/timetable?termYear=${encodeURIComponent(termData.termYear)}&yearLevel=${encodeURIComponent("ปี 1")}&planType=DVE-MSIX`,
          );
          if (timetableRes.ok) {
            const timetableData = await timetableRes.json();
  ...
  async function handleRemoveAssignment(subjectId: number) {
    try {
      ...
      console.log(
        "[FRONTEND] ลบตารางเรียนสำเร็จ (API จะ sync อัตโนมัติ) - วิชา:",
        subject?.subjectName,
      );
    ...
    try {
      ...
      try {
        ...
        const searchResponse = await fetch(
          `/api/subject?subjectCode=${encodeURIComponent(mergedSubject.subjectCode)}&termYear=${encodeURIComponent(termYear || "")}&yearLevel=${encodeURIComponent("ปี 1")}&planType=DVE-LVC`,
        );
        ...
        <div className="bg-card text-card-foreground rounded-xl border my-5 py-6 shadow-sm mx-auto max-w-7xl">
          <div className="flex justify-between items-center mx-8 pb-2 text-lg font-semibold">
            <div className="flex items-center gap-4">
              <span>ตารางเรียน ปวส. ปี 1 (ม.6) ภาคเรียนที่ {termYear}</span>
            </div>
