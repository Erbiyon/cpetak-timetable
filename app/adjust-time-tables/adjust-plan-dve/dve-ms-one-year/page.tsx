...
export default function DveMsixOneYear() {
        ...
        async function fetchData() {
            ...
            try {
                ...
                if (termRes.ok) {
                    ...
                    // ลองแบบ 1: Query ปกติ
                    const query1 = `/api/subject?termYear=${encodeURIComponent(termData.termYear)}&yearLevel=${encodeURIComponent('ปี 1')}&planType=DVE-MSIX`;
    ...
    const handleSubjectUpdate = async () => {
        ...
        try {
            ...
            const response1 = await fetch(`/api/subject?termYear=${encodeURIComponent(currentTermYear)}&yearLevel=${encodeURIComponent('ปี 1')}&planType=DVE-MSIX`);
            if (response1.ok) {
