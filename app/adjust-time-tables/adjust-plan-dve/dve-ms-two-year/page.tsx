...
export default function DveMsixTwoYear() {
    ...
    const handleSubjectUpdate = async () => {
        ...
        try {
            ...
            const timetableRes = await fetch(`/api/timetable?termYear=${encodeURIComponent(termYear || "")}&yearLevel=${encodeURIComponent('ปี 2')}&planType=DVE-MSIX`);
            if (timetableRes.ok) {
                const timetableData = await timetableRes.json();
            ...
            <div className="mx-auto px-4">
                <div className="bg-card text-card-foreground rounded-xl border my-5 py-6 shadow-sm mx-auto max-w-7xl">
                    <div className="flex justify-between mx-8 pb-2 text-lg font-semibold">
                        ตารางเรียน ปวส. ปี 2 (ม.6) ภาคเรียนที่ {termYear}
