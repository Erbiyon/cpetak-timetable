...
export default function DveLvcOneYear() {
        ...
        async function fetchData() {
            ...
            try {
                ...
                if (termRes.ok) {
                    ...
                    const timetableRes = await fetch(`/api/timetable?termYear=${encodeURIComponent(termData.termYear)}&yearLevel=${encodeURIComponent('ปี 1')}&planType=DVE-LVC`);
                    if (timetableRes.ok) {
                ...
                try {
                    ...
                    if (response.status === 409 && data.conflicts) {
                        ...
                        try {
                            ...
                            const searchResponse = await fetch(`/api/subject?subjectCode=${encodeURIComponent(subject.subjectCode)}&termYear=${encodeURIComponent(termYear || '')}&yearLevel=${encodeURIComponent('ปี 1')}&planType=DVE-MSIX`);
    ...
    async function handleMergeSubject(subjectId: number) {
        try {
            ...
            try {
                ...
                // ค้นหาวิชาใน DVE-MSIX ที่มี subjectCode เดียวกัน
                const searchResponse = await fetch(`/api/subject?subjectCode=${encodeURIComponent(mergedSubject.subjectCode)}&termYear=${encodeURIComponent(termYear || '')}&yearLevel=${encodeURIComponent('ปี 1')}&planType=DVE-MSIX`);
    ...
    const handleSubjectUpdate = async () => {
        ...
        try {
            ...
            const timetableRes = await fetch(`/api/timetable?termYear=${encodeURIComponent(termYear || "")}&yearLevel=${encodeURIComponent('ปี 1')}&planType=DVE-LVC`);
            if (timetableRes.ok) {
                const timetableData = await timetableRes.json();
            ...
            <div className="mx-auto px-4">
                <div className="bg-card text-card-foreground rounded-xl border my-5 py-6 shadow-sm mx-auto max-w-7xl">
                    <div className="flex justify-between mx-8 pb-2 text-lg font-semibold">
                        ตารางเรียน ปวช. ขึ้น ปวส. ปี 1 ภาคเรียนที่ {termYear}
