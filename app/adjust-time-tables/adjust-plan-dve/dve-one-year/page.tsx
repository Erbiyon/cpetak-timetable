...
export default function DveLvcOneYear() {
        ...
        async function fetchData() {
            ...
            try {
                ...
                if (termRes.ok) {
                    ...
                    // ลองแบบ 1: Query ปกติ
                    const query1 = `/api/subject?termYear=${encodeURIComponent(termData.termYear)}&yearLevel=${encodeURIComponent('ปี 1')}&planType=DVE-LVC`;
                    ...
                    const query3 = `/api/subject?planType=DVE-LVC`;
    ...
    useEffect(() => {
        if (plans.length > 0) {
            ...
            const year2Plans = plans.filter(plan => plan.yearLevel && plan.yearLevel.includes("ปี 1"));
            console.log("วิชาปี 1:", year2Plans.length);
            console.log("ตัวอย่างวิชาปี 1:", year2Plans.length > 0 ? year2Plans[0] : "ไม่พบ");
        }
    }, [plans]);
    ...
    const handleSubjectUpdate = async () => {
        ...
        try {
            ...
            const response1 = await fetch(`/api/subject?termYear=${encodeURIComponent(currentTermYear)}&yearLevel=${encodeURIComponent('ปี 1')}&planType=DVE-LVC`);
            if (response1.ok) {
