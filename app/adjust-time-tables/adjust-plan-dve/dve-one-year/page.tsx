...
export default function DveLvcOneYear() {
  ...
  const handleSubjectUpdate = useCallback(async () => {
    ...
    try {
      ...
      const timetableRes = await fetch(
        `/api/timetable?termYear=${encodeURIComponent(termYear || "")}&yearLevel=${encodeURIComponent("ปี 1")}&planType=DVE-LVC`,
      );
      if (timetableRes.ok) {
      ...
      const planRes = await fetch(
        `/api/subject?termYear=${encodeURIComponent(termYear || "")}&yearLevel=${encodeURIComponent("ปี 1")}`,
      );
      if (planRes.ok) {
        const allPlanData = await planRes.json();
        const dvePlanData = allPlanData.filter(
          (plan: any) => plan.planType === "DVE-LVC",
        );
    ...
    async function fetchData() {
      ...
      try {
        ...
        if (termRes.ok) {
          ...
          const timetableRes = await fetch(
            `/api/timetable?termYear=${encodeURIComponent(termData.termYear)}&yearLevel=${encodeURIComponent("ปี 1")}&planType=DVE-LVC`,
          );
          if (timetableRes.ok) {
            const timetableData = await timetableRes.json();
      ...
      if (subject) {
        ...
        try {
          ...
          if (response.status === 409 && data.conflicts) {
            ...
            console.log(
              "🎉 [FRONTEND] บันทึกตารางเรียนสำเร็จ (API จะ sync อัตโนมัติ)",
              data,
            );
            setConflicts([]);
  ...
  async function handleRemoveAssignment(subjectId: number) {
    try {
      ...
      console.log(
        "🗑️ [FRONTEND] ลบตารางเรียนสำเร็จ (API จะ sync อัตโนมัติ) - วิชา:",
        subject?.subjectName,
      );
    ...
    try {
      ...
      try {
        ...
        const searchResponse = await fetch(
          `/api/subject?subjectCode=${encodeURIComponent(mergedSubject.subjectCode)}&termYear=${encodeURIComponent(termYear || "")}&yearLevel=${encodeURIComponent("ปี 1")}&planType=DVE-MSIX`,
        );
        ...
        if (searchResponse.ok) {
          ...
          if (splitSubjects.length > 0) {
            console.log(
              `พบวิชาที่แบ่งแล้วใน DVE-MSIX จำนวน ${splitSubjects.length} ส่วน`,
            );
            ...
            if (syncMergeResponse.ok) {
              ...
              console.log(
                "⚠️ ซิ๊งค์การรวมวิชาไปยัง DVE-MSIX ไม่สำเร็จ:",
        ...
        <div className="bg-card text-card-foreground rounded-xl border my-5 py-6 shadow-sm mx-auto max-w-7xl">
          <div className="flex justify-between items-center mx-8 pb-2 text-lg font-semibold">
            <div className="flex items-center gap-4">
              <span>ตารางเรียน ปวส. ปี 1 (ปวช.) ภาคเรียนที่ {termYear}</span>
            </div>
