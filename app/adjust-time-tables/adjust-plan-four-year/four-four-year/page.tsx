...
export default function TransferOneYear() {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            try {
                ...
                if (termYear) {
                    const timetableRes = await fetch(`/api/timetable?termYear=${termYear}&yearLevel=ปี 4&planType=FOUR_YEAR`);
                    if (timetableRes.ok) {
    ...
    useEffect(() => {
        if (plans.length > 0) {
            ...
            const year4Plans = plans.filter(plan => plan.yearLevel && plan.yearLevel.includes("ปี 4"));
            console.log("วิชาปี 4:", year4Plans.length);
            console.log("ตัวอย่างวิชาปี 4:", year4Plans.length > 0 ? year4Plans[0] : "ไม่พบ");
        }
    }, [plans]);
