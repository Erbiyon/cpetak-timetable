  ...
  const handleSubjectUpdate = useCallback(async () => {
    ...
    try {
      ...
      const timetableRes = await fetch(
        `/api/timetable?termYear=${encodeURIComponent(termYear || "")}&yearLevel=${encodeURIComponent("ปี 4")}&planType=FOUR_YEAR`,
      );
      if (timetableRes.ok) {
      ...
      const planRes = await fetch(
        `/api/subject?termYear=${encodeURIComponent(termYear || "")}&yearLevel=${encodeURIComponent("ปี 4")}&planType=FOUR_YEAR`,
      );
      if (planRes.ok) {
    ...
    async function fetchData() {
      ...
      try {
        ...
        if (termRes.ok) {
          ...
          const timetableRes = await fetch(
            `/api/timetable?termYear=${encodeURIComponent(termData.termYear)}&yearLevel=${encodeURIComponent("ปี 4")}&planType=FOUR_YEAR`,
          );
          if (timetableRes.ok) {
            const timetableData = await timetableRes.json();
  ...
  useEffect(() => {
    if (plans.length > 0) {
      ...
      const year1Plans = plans.filter(
        (plan) => plan.yearLevel && plan.yearLevel.includes("ปี 4"),
      );
        ...
        <div className="bg-card text-card-foreground rounded-xl border my-5 py-6 shadow-sm mx-auto max-w-7xl">
          <div className="flex justify-between items-center mx-8 pb-2 text-lg font-semibold">
            <div className="flex items-center gap-4">
              <span>ตารางเรียน 4 ปี ปี 4 ภาคเรียนที่ {termYear}</span>
            </div>
