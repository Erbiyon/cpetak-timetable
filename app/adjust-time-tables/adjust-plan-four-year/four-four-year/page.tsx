...
export default function FourFourYear() {
        ...
        async function fetchData() {
            ...
            try {
                ...
                if (termRes.ok) {
                    ...
                    const timetableRes = await fetch(`/api/timetable?termYear=${encodeURIComponent(termData.termYear)}&yearLevel=${encodeURIComponent('ปี 4')}&planType=FOUR_YEAR`);
                    if (timetableRes.ok) {
                    ...
                    const query1 = `/api/subject?termYear=${encodeURIComponent(termData.termYear)}&yearLevel=${encodeURIComponent('ปี 4')}&planType=FOUR_YEAR`;
                    ...
                    if (finalData.length === 0) {
                        ...
                        if (planRes.ok) {
                            ...
                            finalData = allData.filter((record: any) => {
                                ...
                                const matchYearLevel = record.yearLevel === 'ปี 4';
    ...
    useEffect(() => {
        if (plans.length > 0) {
            ...
            const year1Plans = plans.filter(plan => plan.yearLevel && plan.yearLevel.includes("ปี 4"));
            console.log("วิชาปี 4:", year1Plans.length);
            console.log("ตัวอย่างวิชาปี 4:", year1Plans.length > 0 ? year1Plans[0] : "ไม่พบ");
        }
    }, [plans]);
                ...
                try {
                    ...
                    const response = await fetch('/api/timetable', {
                        ...
                        body: JSON.stringify({
                            planId: subjectId,
                            termYear: termYear || '1',
                            yearLevel: 'ปี 4', // เปลี่ยนตามแต่ละไฟล์
    ...
    const handleSubjectUpdate = async () => {
        ...
        try {
            ...
            const timetableRes = await fetch(`/api/timetable?termYear=${encodeURIComponent(termYear || "")}&yearLevel=${encodeURIComponent('ปี 4')}&planType=FOUR_YEAR`);
            if (timetableRes.ok) {
                const timetableData = await timetableRes.json();
            ...
            const planRes = await fetch(`/api/subject?termYear=${encodeURIComponent(termYear || "")}&yearLevel=${encodeURIComponent('ปี 4')}&planType=FOUR_YEAR`);
            if (planRes.ok) {
            ...
            <div className="mx-auto px-4">
                <div className="bg-card text-card-foreground rounded-xl border my-5 py-6 shadow-sm mx-auto max-w-7xl">
                    <div className="flex justify-between mx-8 pb-2 text-lg font-semibold">
                        ตารางเรียน 4 ปี ปี 4 ภาคเรียนที่ {termYear}
