...
export default function FourOneYear() {
        ...
        async function fetchData() {
            ...
            try {
                ...
                if (termRes.ok) {
                    ...
                    const timetableRes = await fetch(`/api/timetable?termYear=${encodeURIComponent(termData.termYear)}&yearLevel=${encodeURIComponent('ปี 1')}&planType=FOUR_YEAR`);
                    if (timetableRes.ok) {
    ...
    const handleSubjectUpdate = async () => {
        ...
        try {
            ...
            const timetableRes = await fetch(`/api/timetable?termYear=${encodeURIComponent(termYear || "")}&yearLevel=${encodeURIComponent('ปี 1')}&planType=FOUR_YEAR`);
            if (timetableRes.ok) {
                const timetableData = await timetableRes.json();
            ...
            const planRes = await fetch(`/api/subject?termYear=${encodeURIComponent(termYear || "")}&yearLevel=${encodeURIComponent('ปี 1')}&planType=FOUR_YEAR`);
            if (planRes.ok) {
            ...
            <div className="mx-auto px-4">
                <div className="bg-card text-card-foreground rounded-xl border my-5 py-6 shadow-sm mx-auto max-w-7xl">
                    <div className="flex justify-between mx-8 pb-2 text-lg font-semibold">
                        ตารางเรียน 4 ปี ปี 1 ภาคเรียนที่ {termYear}
