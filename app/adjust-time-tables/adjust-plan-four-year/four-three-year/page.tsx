...
export default function FourThreeYear() {
        ...
        async function fetchData() {
            ...
            try {
                ...
                if (termRes.ok) {
                    ...
                    const timetableRes = await fetch(`/api/timetable?termYear=${encodeURIComponent(termData.termYear)}&yearLevel=${encodeURIComponent('ปี 3')}&planType=FOUR_YEAR`);
                    if (timetableRes.ok) {
                    ...
                    // ลองแบบ 1: Query ปกติ
                    const query1 = `/api/subject?termYear=${encodeURIComponent(termData.termYear)}&yearLevel=${encodeURIComponent('ปี 3')}&planType=FOUR_YEAR`;
    ...
    useEffect(() => {
        if (plans.length > 0) {
            ...
            const year2Plans = plans.filter(plan => plan.yearLevel && plan.yearLevel.includes("ปี 3"));
            console.log("วิชาปี 3:", year2Plans.length);
            console.log("ตัวอย่างวิชาปี 3:", year2Plans.length > 0 ? year2Plans[0] : "ไม่พบ");
        }
    }, [plans]);
    ...
    const handleSubjectUpdate = async () => {
        ...
        try {
            ...
            const response1 = await fetch(`/api/subject?termYear=${encodeURIComponent(currentTermYear)}&yearLevel=${encodeURIComponent('ปี 3')}&planType=FOUR_YEAR`);
            if (response1.ok) {
