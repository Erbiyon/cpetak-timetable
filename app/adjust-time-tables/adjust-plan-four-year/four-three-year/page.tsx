...
export default function FourTreeYear() {
    ...
    const handleSubjectUpdate = async () => {
        ...
        try {
            ...
            const timetableRes = await fetch(`/api/timetable?termYear=${encodeURIComponent(termYear || "")}&yearLevel=${encodeURIComponent('ปี 3')}&planType=FOUR_YEAR`);
            if (timetableRes.ok) {
                const timetableData = await timetableRes.json();
            ...
            const planRes = await fetch(`/api/subject?termYear=${encodeURIComponent(termYear || "")}&yearLevel=${encodeURIComponent('ปี 3')}&planType=FOUR_YEAR`);
            if (planRes.ok) {
