...
export default function AcademicCalendar() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          data.forEach((term: any) => {
            if (term.name === "ภาคเรียนที่ 1")
              setTerm1({
                start: new Date(term.start),
                end: new Date(term.end),
              });
            if (term.name === "ภาคเรียนที่ 2")
              setTerm2({
                start: new Date(term.start),
                end: new Date(term.end),
              });
            if (term.name === "ภาคเรียนที่ 3")
              setTerm3({
                start: new Date(term.start),
                end: new Date(term.end),
              });
  ...
  React.useEffect(() => {
    if (term1.start && term1.end)
      saveTerm("ภาคเรียนที่ 1", term1.start, term1.end);
  }, [term1.start, term1.end]);

  React.useEffect(() => {
    if (term2.start && term2.end)
      saveTerm("ภาคเรียนที่ 2", term2.start, term2.end);
  }, [term2.start, term2.end]);

  React.useEffect(() => {
    if (term3.start && term3.end)
      saveTerm("ภาคเรียนที่ 3", term3.start, term3.end);
  ...
  return (
    <div className="flex justify-center">
      <div className="bg-card text-card-foreground flex flex-col gap-2 rounded-xl m-4 lg:m-10 shadow-sm max-w-3xl w-full">
        <div className="text-center text-xl lg:text-2xl font-bold my-4 lg:my-3 px-4">
          ปฏิทินการศึกษา
        </div>
          ...
          <div className="w-full">
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 lg:gap-6">
              <div className="flex flex-col space-y-3">
                <Label
                  htmlFor="term1"
                  className="px-1 text-sm lg:text-base font-medium"
                >
                  ภาคเรียนที่ 1
                </Label>
                <div className="space-y-2">
                  <CalendarCustom
                    ...
                    onChange={(date) => {
                      ...
                      const overlaps = getAllOverlapTermNames(
                        newStart,
                        newEnd,
                        [
        ...
        <Card className="mx-4 mb-4">
          <CardHeader>
            <CardTitle className="text-lg font-semibold">
              กำหนดภาคเรียนปัจจุบัน
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                เลือกภาคเรียนที่ต้องการให้เป็นภาคเรียนปัจจุบัน
                (จะส่งผลต่อการแสดงข้อมูลทั้งระบบ)
              </p>
                      ...
                      <div key={termNumber} className="flex-1">
                        ...
                        >
                          <div className="flex flex-col items-center">
                            <span>
                              ภาคเรียนที่ {termNumber}/{academicYear}
                            </span>
                            {!isComplete && (
