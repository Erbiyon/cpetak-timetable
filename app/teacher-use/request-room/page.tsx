...
export default function RequestRoomPage() {
  ...
  useEffect(() => {
    ...
    if (
      !accessDenied &&
      teacherInfo &&
      teacherInfo.teacherType !== "อาจารย์ภายนอกสาขา"
    ) {
      fetchTermYear();
    }
  ...
  const handleRoomUpdate = async (subjectId: number, roomId: string) => {
    ...
    try {
      ...
      const isDVE =
        subject.planType === "DVE-MSIX" || subject.planType === "DVE-LVC";
      ...
      <div className="mb-6">
        ...
        {teacherInfo && (
          <div className="mt-2 text-sm text-gray-500">
            อาจารย์: {teacherInfo.tName} {teacherInfo.tLastName} (
                ...
                {isDeadlinePassed ? (
                  <>
                    <p className="font-semibold text-red-700">
                      การเลือกห้องเรียนถูกปิดแล้ว
                    </p>
                    <p className="text-sm text-red-600 mt-1">
                      วันที่กำหนดสิ้นสุดเมื่อ: {formatThaiDate(deadline)}
                    </p>
                    <p className="text-sm text-muted-foreground mt-1">
                      หากต้องการเปลี่ยนแปลงห้องเรียน กรุณาติดต่อผู้ดูแลระบบ
                    </p>
                  </>
                ) : (
                  <>
                    <p className="font-semibold text-yellow-700">
                      การเลือกห้องเรียนจะปิดในวันที่: {formatThaiDate(deadline)}
                    </p>
                    <p className="text-sm text-muted-foreground mt-1">
                      กรุณาเลือกห้องเรียนให้เสร็จสิ้นก่อนวันที่กำหนด
                    </p>
                  </>
                )}
              </div>
                        ...
                        <Table>
                          <TableHeader>
                            <TableRow>
                              ...
                              <TableHead className="w-[340px] text-center">
                                ห้องเรียน
                              </TableHead>
                              <TableHead className="w-[80px] text-center">
                                สถานะ
                              ...
                              >
                                ...
                                <TableCell>
                                  <Select
                                    ...
                                    onValueChange={(value) =>
                                      handleRoomUpdate(subject.id, value)
                                    }
                                    disabled={
                                      updating === subject.id ||
                                      isDeadlinePassed
