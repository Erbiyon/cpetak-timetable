...
export async function POST(request: Request) {
  try {
    ...
    const baseSubjectName = subjectToMerge.subjectName.replace(
      /\s*\(ส่วนที่ \d+\)\s*$/,
      "",
    );
    ...
    if (coTeachingGroup) {
      ...
      console.log(
        "Related co-teaching groups (all year levels):",
        relatedCoTeachingGroups.map((g) => ({
          id: g.id,
          groupKey: g.groupKey,
      ...
      const allPlansToMerge = relatedCoTeachingGroups.flatMap(
        (group) => group.plans,
      );

      console.log(
        "All plans to merge (all year levels):",
        allPlansToMerge.map((p) => ({
          id: p.id,
          planType: p.planType,
          yearLevel: p.yearLevel,
          subjectName: p.subjectName,
      ...
      if (allPlansToMerge.length === 0) {
        return NextResponse.json(
          { error: "ไม่พบวิชาที่สามารถรวมได้" },
          { status: 400 },
        );
      }

      const groupedByPlanTypeAndYear = allPlansToMerge.reduce(
        (acc, plan) => {
      ...
      console.log(
        "Grouped plans:",
        Object.entries(groupedByPlanTypeAndYear).map(([key, plans]) => ({
          key,
      ...
      for (const [, plans] of Object.entries(groupedByPlanTypeAndYear)) {
        if (plans.length > 0) {
          const result = await mergeSubjectPartsWithDeleted(
            plans,
            baseSubjectName,
          );
      ...
      if (splitParts.length < 2) {
        return NextResponse.json(
          { error: "ไม่พบส่วนที่แบ่งเพื่อรวม" },
          { status: 400 },
        );
      }
...
async function mergeSubjectParts(splitParts: any[], baseSubjectName: string) {
  ...
  const totalLectureHours = splitParts.reduce(
    (sum, part) => sum + (part.lectureHour || 0),
    0,
  );
  const totalLabHours = splitParts.reduce(
    (sum, part) => sum + (part.labHour || 0),
    0,
  );
...
async function mergeSubjectPartsWithDeleted(
  splitParts: any[],
