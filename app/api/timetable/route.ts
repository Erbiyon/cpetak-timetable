...
export async function POST(request: Request) {
  try {
    ...
    const isTransferOrFourYear =
      planType === "TRANSFER" || planType === "FOUR_YEAR";
    ...
    if (isTransferOrFourYear) {
      ...
      isCoTeaching =
        coTeachingGroup !== null && coTeachingGroup.plans.length > 1;
    }
    ...
    if (shouldCheckConflicts) {
      ...
      if (section) {
        ...
        if (currentPlan) {
          const currentIsDVE =
            currentPlan.planType === "DVE-MSIX" ||
            currentPlan.planType === "DVE-LVC";
          ...
          if (filteredDuplicates.length > 0) {
            const conflictDetails = filteredDuplicates
              .map((plan) => {
                const planTypeText =
                  plan.planType === "TRANSFER"
                    ? "เทียบโอน"
                    : plan.planType === "FOUR_YEAR"
                      ? "4 ปี"
            ...
            conflicts.push({
              ...
              message: `วิชา ${currentPlan.subjectCode} กลุ่มเรียน ${section} มีอยู่แล้วในแผนการเรียน: ${conflictDetails}`,
    ...
    if (isDVE) {
      ...
      if (currentPlan) {
        const isSplitSubject =
          ...
          currentPlan.subjectName.includes("(ส่วนที่");

        if (isSplitSubject) {
          console.log(
            `� [API-SYNC] ซิ๊งค์วิชาที่แบ่งแล้ว: ${currentPlan.subjectName} (planId: ${planId})`,
          );
          ...
          console.log(
            `🎯 [API-SYNC] พบวิชาส่วนเดียวกัน ${duplicatePlans.length} รายการ`,
          );
          ...
          console.log(
            `🔄 [API-SYNC] ทำการซิ๊งค์วิชาปกติ: ${currentPlan.subjectName} (planId: ${planId})`,
          );
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("API: กำลังดึงข้อมูลตารางเรียนสำหรับ:", {
      roomId,
      teacherId,
      termYear,
      yearLevel,
    ...
    const timetables = await prisma.timetable_tb.findMany({
      ...
      orderBy: [{ day: "asc" }, { startPeriod: "asc" }],
    });
